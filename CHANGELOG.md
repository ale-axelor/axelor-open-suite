...
* PFP : Add verification if Passed For Payment is enable in company configuration and application configuration
* Stock : Fix stock indicators computation
...
* Invoice : Fix an error while paying a customer invoice with holdback
* Inventory : Error when trying to open a line
* Payment voucher : Fix threshold payment when invoice term percentages have been customized
* [PROJECT] Wrong fr translation for 'Create business project from this template'
* SALEORDER : Error during confirmation
* Payment session : Set currency field uneditable on payment session
* Period : Improve period closure process from year form
* [SALE ORDER] Advance Payment panel issue
* ProdProcessLine: Fixed a bug where work center's durations were used instead of prod process line.
* Group by product not working on purchase order generated from purchase requests
* Sequence : fix Duplication of sequence not working - java null pointer
* Move : Fix an error when we create a move line with a partner without any accounting situation
* Advance Payment Invoice : Fix error when trying to pay
* [SOP/MPS] : Fixed a bug where a existing Forecast was modified instead of creating a new one when on different date
* Move : Error when trying to open an invoice term
